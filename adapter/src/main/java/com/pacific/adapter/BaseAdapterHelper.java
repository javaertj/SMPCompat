...
//        ImageLoader.getInstance().displayImage(imageUrl,view);
    ...
    public T setImageUrl(int viewId, String imageUrl,int res) {
        ImageView view = retrieveView(viewId);

        Glide.with(view.getContext()).load(imageUrl).placeholder(res).diskCacheStrategy(DiskCacheStrategy.ALL).into(view);
