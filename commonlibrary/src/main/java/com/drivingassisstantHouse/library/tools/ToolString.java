...
public class ToolString {
    ...
    /***
     * 实现不同色的文本处理
     * @param source 要处理的文本，不能为null或空串
     * @param colorStart 差异色开始索引
     * @param colorEnd 差异色结束所以
     * @param color 差异色
     * @return
     */
    public static SpannableStringBuilder createSpannableString(CharSequence source, int colorStart, int colorEnd,int color) {
        if (TextUtils.isEmpty(source)) {
            return null;
        }
        SpannableStringBuilder style = new SpannableStringBuilder(source);
        if (colorStart > 0 && colorEnd > 0&&colorEnd>colorStart) {
            style.setSpan(new ForegroundColorSpan(color), colorStart, colorEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);//实现部分文字颜色改变

            AbsoluteSizeSpan span = new AbsoluteSizeSpan(ToolUnit.dipTopx(11));
            style.setSpan(span, colorStart, colorEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE );
        }
        return style;
    }
}
