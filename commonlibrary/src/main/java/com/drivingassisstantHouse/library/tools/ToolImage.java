...
public abstract class ToolImage {
     ...
     * @param cachePath 缓存图片的目录
     * @param defaultDrawable
     * @return
     */
    public static void init(Context context,String cachePath,int defaultDrawable) {
        DisplayImageOptions defaultOptions = new DisplayImageOptions.Builder()
                .showImageOnLoading(defaultDrawable)
                .showImageForEmptyUri(defaultDrawable)// 空uri时的默认图片
                .showImageOnFail(defaultDrawable)// 加载失败时的默认图片
...
//                  .memoryCache(new WeakMemoryCache())
                .memoryCacheSize(10 * 1024 * 1024)//内存缓存10MB
