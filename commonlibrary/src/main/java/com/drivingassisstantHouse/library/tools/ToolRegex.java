...
import static com.drivingassisstantHouse.library.tools.ToolRegex.StringType.HTTP;
import static com.drivingassisstantHouse.library.tools.ToolRegex.StringType.OPEN_APP;
...
public final class ToolRegex {
    ...
    private static final String regexMobilePhone = "(\\+\\d+)?1[34578]\\d{9}$";// /^0?(13|14|15|17|18)[0-9]{9}$/
    //泰国手机号正则
...
//        }
        ...
        if (stringType.equals(StringType.ENGLISH_NAME)) {
            regexk = regexkEnglish;
        }
        if (stringType.equals(StringType.CHINESE)) {
            regexk = regexkURL;
        }
        ...
        if (stringType.equals(OPEN_APP)) {
            regexk = regexSimpleTourAppPact;
        }
        if (stringType.equals(HTTP)) {
            regexk = regexHttpUrl;
        }
        ...
        if (stringType.equals(StringType.MOBILEL)) {
            return Pattern.matches(regexMobilePhone, string) || Pattern.matches(regexMobilePhoneTaiGuo, string);
        } else if (!TextUtils.isEmpty(regexk)) {
